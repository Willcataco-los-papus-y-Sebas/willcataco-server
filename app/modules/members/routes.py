...
@router.post("/", status_code = status.HTTP_201_CREATED, response_model = IResponse, dependencies=[Depends(RequireRoles(UserRole.ADMIN))])
...
@router.get("/{id}", status_code = status.HTTP_200_OK, response_model = IResponse, dependencies=[Depends(RequireRoles(UserRole.ADMIN))])
...
@router.delete("/{id}", status_code=status.HTTP_200_OK, response_model= IResponse, dependencies=[Depends(RequireRoles(UserRole.ADMIN))])
async def delete_member(session: SessionDep, id: int):
    return await MemberController.delete_member(session, id)

@router.get("/", status_code= status.HTTP_200_OK, response_model = IResponse, dependencies=[Depends(RequireRoles(UserRole.ADMIN))])
