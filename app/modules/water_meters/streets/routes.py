...
@router.post("/", status_code = status.HTTP_201_CREATED, response_model= IResponse, dependencies=[RequireRoles(UserRole.ADMIN, UserRole.STAFF)])
...
@router.get("/", status_code= status.HTTP_200_OK, response_model= IResponse, dependencies=[RequireRoles(UserRole.ADMIN, UserRole.STAFF)])
async def get_all_streets(session: SessionDep):
    return await StreetControllers.get_all_streets(session)


@router.get("/{id}", status_code= status.HTTP_200_OK, response_model= IResponse, dependencies=[RequireRoles(UserRole.ADMIN, UserRole.STAFF)])
async def read_street(session: SessionDep, id: int):
    return await StreetControllers.read_street(session, id)


@router.patch("/{id}", status_code= status.HTTP_200_OK, response_model= IResponse, dependencies=[RequireRoles(UserRole.ADMIN, UserRole.STAFF)])
...
@router.delete("/{id}", status_code= status.HTTP_200_OK, response_model= IResponse, dependencies=[RequireRoles(UserRole.ADMIN, UserRole.STAFF)])
