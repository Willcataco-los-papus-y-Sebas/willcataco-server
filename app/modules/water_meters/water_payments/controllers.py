...
class WaterPaymentController:
    ...
    async def create_water_payment(session: SessionDep, payment_info: WaterPaymentBase):
        ...
        response = IResponse(
            detail="Water payment created", status_code=201, data=payment
        )
        return response
    ...
    async def change_status(id: int, session: SessionDep):
        ...
        response = IResponse(
            detail="Water payment status updated", status_code=200, data=updated_payment
        )
        return response
    ...
    async def read_water_payment(id: int, session: SessionDep):
        ...
        if not payment:
            raise HTTPException(status_code=404, detail="water payment not found")
        response = IResponse(
            detail="Water payment retrieved", status_code=200, data=payment
        )
        return response
