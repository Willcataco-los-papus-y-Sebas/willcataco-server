...
from app.core.time import get_time_zone
...
class EmailService:
    ...
    async def send_water_bill_email(
        ...
        try:
            ...
            template_context["date"] = get_time_zone(bill_data.date).strftime("%Y-%m-%d")
    ...
    async def send_water_payment_email(
        ...
        try:
            ...
            dump_receipt["date_created"] = get_time_zone(email_receipt.date_created).strftime(
                "%d/%m/%Y"
            )
            dump_receipt["date_updated"] = get_time_zone(email_receipt.date_updated).strftime(
                "%d/%m/%Y  %H:%M"
            )
